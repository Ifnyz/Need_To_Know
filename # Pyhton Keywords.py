...
with open('file.txt', 'r') as f:
    data = f.read() # do something with the data
# the file is automatically closed here
# In the example above, the with keyword is used to open the file file.txt in read-only mode. The file is automatically closed at the end of the with block, so you don't have to worry about closing it manually.
# The with keyword is often used when working with external resources, such as files or network connections, because it ensures that the resource is properly cleaned up (i.e. closed or released) when it is no longer needed. This can help prevent errors and resource leaks.
# Note that the with keyword is not specific to working with files, and it can be used with any object that has a __enter__ and __exit__ method. These methods are called when the with block is entered and exited, respectively, and they can be used to set up and tear down the resource being used.
...
# 2
# 3
# In the example above, the my_generator function is a generator that yields the values 1, 2, and 3. When the for loop iterates over the generator, the generator yields each value, which is printed to the console.
# The yield keyword is used to define a generator function, which is a special type of function that returns an iterator. When a generator function is called, it doesn't actually execute the function body, but instead returns an iterator that can be used to iterate over the values generated by the generator.
# The yield keyword is used inside the generator function to yield a value to the caller. When the generator yields a value, the function is paused, and the yielded value is returned to the caller. When the caller requests the next value from the iterator, the generator function is resumed, and it continues executing until the next yield statement is reached. This continues until the generator function completes, at which point the iterator is exhausted and the iteration is finished.
# Generators are a powerful and efficient way to work with sequences of values in Python, and the yield keyword is an important part of how they work.
